/*
  A 2D Schnakenberg reaction-diffusion simulation on a NX x NY grid,
  using an IMEX time scheme, Gauss-Seidel for diffusion, and OpenMP parallelization.
  Outputs CSV snapshots of A and B every certain number of steps for downstream analysis via Python scripts.

  NOTE: This code includes cross difussions terms and a more elaborate reaction time-step
...
static const double DB = 100.0;  // Diffusion of B
static const double alpha = 0.05; // feed rate of activator
static const double beta  = 0.25; // feed rate of inhibitor
static const double chi_a = 1.0;
static const double chi_b = 10.0;

// Time stepping
static const double dt = 0.004;   // time step
static const int STEPS = 60000;   // total number of steps
static const int OUTPUT_INTERVAL = 50; // output every 20 steps
...
void diffuseGaussSeidel(std::vector<double> &X, const std::vector<double> &RHS,
                        double diffCoeff)
{
    // We solve (X - dt*diffCoeff*Lap(X)) = RHS (RHS is temporary field after reaction + cross-diffusion step.)
    // => X_new(i,j) = (RHS + dt*diffCoeff*(neighbors)) / (1 + 4*r)
    // We do multiple sweeps. Red-Black approach for parallelization.
...
int main()
{
    ...
    std::vector<double> lapA(NX*NY), lapB(NX*NY);
    std::vector<double> RHS_A(NX*NY), RHS_B(NX*NY);
    ...
    for(int step=0; step<=STEPS; step++){
        // ======= Reaction step + Cross Diffussion (explicit) =======
        // A^* = A^n + dt*(alpha*A*(1-r1*B^2) + B*(1-r2*A))
        // B^* = B^n + dt*(beta*A*(1+r1*a*b) + a*(gamma + r2*b)
        #pragma omp parallel for collapse(2)
        for(int j=0; j<NY; j++){
            for(int i=0; i<NX; i++){
                int id = idx(i,j);
        ...
        // precompute discrete Laplacians of Atemp and Btemp
        #pragma omp parallel for collapse(2)
        for(int j = 0; j < NY; ++j) {
            for(int i = 0; i < NX; ++i) {
                int id = idx(i,j);
                int ip = wrap(i+1,NX), im = wrap(i-1,NX);
                int jp = wrap(j+1,NY), jm = wrap(j-1,NY);
                lapA[id] = Atemp[idx(ip,j)]+ Atemp[idx(im,j)]+ Atemp[idx(i,jp)]+ Atemp[idx(i,jm)]- 4.0 * Atemp[id];
                lapB[id] = Btemp[idx(ip,j)]+ Btemp[idx(im,j)]+ Btemp[idx(i,jp)]+ Btemp[idx(i,jm)]- 4.0 * Btemp[id];
            }
        }

        // build the RHS including explicit cross‑diffusion
        #pragma omp parallel for collapse(2)
        for(int j = 0; j < NY; ++j) {
            for(int i = 0; i < NX; ++i) {
                int id = idx(i,j);
                RHS_A[id] = Atemp[id] + dt * chi_a * lapB[id];
                RHS_B[id] = Btemp[id] + dt * chi_b * lapA[id];
            }
        }

        // now solve (I - dt*Du*Lap) Aⁿ⁺¹ = RHS_A  (and same for B)
        diffuseGaussSeidel(A, RHS_A, DA);
        diffuseGaussSeidel(B, RHS_B, DB);
        ...
        //std::cout<<"Step "<<step<<": A in ["<<A_min<<","<<A_max<<"]\n";
