...
int main() {
    ...
    for (int N_BETA = -25; N_BETA < 26; N_BETA += 1) {
        for (int N_ALPHA = -25; N_ALPHA < 26; N_ALPHA += 1) {
            ...
            for (int j = 0; j < NY; j++){
                for (int i = 0; i < NX; i++){
                    double noiseA = 0.1 * (((double)rand()/RAND_MAX));
                    double noiseB = 0.1 * (((double)rand()/RAND_MAX));
                    A[idx(i,j)] = A0 + noiseA;
                    B[idx(i,j)] = B0 + noiseB;
                }
            }
            ...
            #pragma omp critical
            fout << std::fixed << std::setprecision(4) << alpha << ","
                 << std::fixed << std::setprecision(4) << beta << ","
