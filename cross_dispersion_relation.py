...
w = dispersion_relation_pos(k_complex)
w_neg = dispersion_relation_neg(k_complex)

k_max_idx = np.argmax(w.real)
k_max = k[k_max_idx]
w_max = w[k_max_idx]
w_imag_max = w.imag[k_max_idx]
print(f"Maximum real part of w: {w_max} at k = {k_max}")
print(f"Maximum imaginary part of w: {w_imag_max} at k = {k_max}")
...
param_texts = [
    ...
    fr'$\frac{{Chi_A}}{{Chi_b}}$ = {Xa/Xb:.2f}',
    fr'$k_{{\max}}$ = {k_max:.2f}',
    fr'$\omega_{{\max}}$ = {w_max:.2f}',
]

fig.text(0.87, 0.5, '\n'.join(param_texts), fontsize='medium', ha='left', va='center')
plt.savefig(f'cross{Xa/Xb}_a{alpha:.2f}_b{beta:.2f}_diff{(Da/Db):.2f}.png', dpi=300)
plt.show()